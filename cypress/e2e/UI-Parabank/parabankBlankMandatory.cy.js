...
describe("Parabank Registration", { testIsolation: false }, () => {
  before(() => {
    ...
    it("Verify blank First Name displays an error message in the Update Profile page", () => {
      ...
      cy.get("[id='customer.lastName']").should('be.visible').invoke('val').should('eq', testData.lastName);
      cy.get("[id='customer.address.street']").should('be.visible').invoke('val').should('eq', testData.address);
      cy.get("[id='customer.address.city']").should('be.visible').invoke('val').should('eq', testData.city);
      cy.get("[id='customer.address.state']").should('be.visible').invoke('val').should('eq', testData.state);
      cy.get("[id='customer.address.zipCode']").should('be.visible').invoke('val').should('eq', testData.zipCode);
      cy.get("[id='customer.phoneNumber']").should('be.visible').invoke('val').should('eq', testData.phoneNumber);
    ...
    it("Verify blank Last Name displays an error message in the Update Profile page", () => {
      ...
      cy.get("[id='customer.firstName']").should('be.visible').invoke('val').should('eq', testData.firstName);
      cy.get("[id='customer.address.street']").should('be.visible').invoke('val').should('eq', testData.address);
      cy.get("[id='customer.address.city']").should('be.visible').invoke('val').should('eq', testData.city);
      cy.get("[id='customer.address.state']").should('be.visible').invoke('val').should('eq', testData.state);
      cy.get("[id='customer.address.zipCode']").should('be.visible').invoke('val').should('eq', testData.zipCode);
  ...
  it("Verify Phone Number is an optional field", () => {
     cy.get('#leftPanel > ul > :nth-child(6) > a').should('be.visible').and('contain', "Update Contact Info").click();
      ...
      cy.wait(1000);
      cy.get("[id='customer.phoneNumber']").clear().should('have.value', '');
      // cy.get("[id='customer.firstName']").should('be.visible').invoke('val').should('eq', Userinput.firstName);
      // cy.get("[id='customer.lastName']").should('be.visible').invoke('val').should('eq', Userinput.lastName);
      // cy.get("[id='customer.address.street']").should('be.visible').invoke('val').should('eq', Userinput.address);
      // cy.get("[id='customer.address.city']").should('be.visible').invoke('val').should('eq', Userinput.city);
      // cy.get("[id='customer.address.state']").should('be.visible').invoke('val').should('eq', Userinput.state);
      // cy.get("[id='customer.zipCode']").should('be.visible').invoke('val').should('eq', Userinput.zipCode);
      cy.get('input.button[value="Update Profile"]').should('be.visible').click();
